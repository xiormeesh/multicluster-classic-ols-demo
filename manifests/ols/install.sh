...
if oc get olsconfig cluster -o name &>/dev/null && \
   oc get deployment lightspeed-app-server -n "$NS" -o name &>/dev/null; then
...
oc wait --for=jsonpath='{.status.phase}'=Succeeded csv -n "$NS" \
    -l operators.coreos.com/lightspeed-operator.openshift-lightspeed \
    --timeout=300s

if oc get secret credentials -n "$NS" -o name &>/dev/null; then
